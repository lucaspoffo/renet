...
use web_sys::{WebTransport, ReadableStreamDefaultReader, WebTransportOptions, WebTransportError};
...
impl Client {
    pub async fn new(url: &str, options: Option<WebTransportOptions>) -> Result<Self, WebTransportError> {
        let webtransport = Self::init_web_transport(url, options).await?;
        Ok(Self { web_transport: webtransport})
    }

    pub async fn update(&mut self, renet_client: &mut RenetClient) {
        let reader: ReadableStreamDefaultReader  = JsValue::from(self.web_transport
            .datagrams().readable().get_reader()).into();
        loop {
            let (done, value): (bool, Uint8Array) = JsFuture::from(reader.read()).await.and_then(|js_value| {
                let done = js_sys::Reflect::get(&js_value, &JsValue::from_str("done")).unwrap().as_bool().unwrap();
                let value: Uint8Array = <JsValue as Into<Uint8Array>>::into(js_sys::Reflect::get(&js_value , &JsValue::from_str("value")).unwrap());
                Ok((done, value))
            }).unwrap();
            if done {
                break;
    ...
    pub async fn send_packets(&mut self, renet_client: &mut RenetClient) {
        ...
        let writer = self.web_transport
            .datagrams().writable().get_writer().unwrap();
    ...
    pub async fn disconnect(self) -> Result<(), WebTransportCloseError> {
        ...
        if close_result.is_err() {
            ...
            let code = js_sys::Reflect::get(&closed_js_value, &JsValue::from_str("code")).unwrap().as_f64().unwrap();
            let reason = js_sys::Reflect::get(&closed_js_value, &JsValue::from_str("reason")).unwrap().as_string().unwrap();
