...
pub fn draw_loader(ui: &mut egui::Ui) {
    ...
    for server_addr in chat_server.transport.addresses() {
        ui.horizontal(|ui| {
            ...
            if ui.button("📋").on_hover_text(tooltip).clicked() {
                ui.ctx().copy_text(server_addr.to_string());
            }
        });
    }
...
pub fn draw_main_screen(ui_state: &mut UiState, state: &mut AppState, ui: &mut egui::Ui) {
    egui::CentralPanel::default().show_inside(ui, |ui| {
...
pub fn draw_chat(ui_state: &mut UiState, state: &mut AppState, usernames: HashMap<ClientId, String>, ui: &mut egui::Ui) {
    if ui_state.show_network_info {
        match state {
            AppState::ClientChat { visualizer, .. } => {
                visualizer.show_window(ui.ctx());
            }
            AppState::HostChat { ref mut chat_server } => {
                chat_server.visualizer.show_window(ui.ctx());
            }
            _ => {}
        }
    }
    ...
    egui::Panel::bottom("text_editor").show_inside(ui, |ui| {
