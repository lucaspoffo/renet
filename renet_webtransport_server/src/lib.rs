use std::{path::PathBuf, io::Error, sync::Arc, time::Duration, net::SocketAddr, collections::HashMap};
use bytes::{BufMut, Bytes, BytesMut};
use h3::{server::Connection, ext::Protocol, error::ErrorLevel};
...
impl WebTransportServer {
    pub async fn new(config: WebTransportConfig) -> Result<Self, Error> {
        let addr = config.listen;
        let server_config = Self::create_server_config(config)?;
        let endpoint = quinn::Endpoint::server(server_config, addr)?;
        
        Ok(Self {endpoint, clients: HashMap::new(), client_iterator: 0})
    }
    ...
    pub async fn update(&mut self, renet_server: &mut RenetServer) {
        ...
        for (client_id, session) in self.clients.iter_mut() {
            ...
            if let Some((_, datagram_bytes)) = datagram.await.unwrap() {
                let _ = renet_server.process_packet_from(&datagram_bytes, client_id.clone());
            }
        }
    ...
    fn create_server_config(config: WebTransportConfig) -> Result<quinn::ServerConfig, std::io::Error> {
        ...
        let mut tls_config = rustls::ServerConfig::builder()
            ...
            .unwrap()
            .with_no_client_auth()
            .with_single_cert(vec![cert], key).unwrap(); // TODO handle Errorcase
    ...
    async fn handle_connection(mut conn: Connection<h3_quinn::Connection, Bytes>) -> Result<Option<WebTransportSession<h3_quinn::Connection, Bytes>>, h3::Error> { 
            ...
            Ok(Some((req, stream))) => {
                ...
                match req.method() {
                    &Method::CONNECT if ext.get::<Protocol>() == Some(&Protocol::WEB_TRANSPORT) => {
                        ...
                        let session: WebTransportSession<h3_quinn::Connection, Bytes> = WebTransportSession::accept(req, stream, conn).await?;
