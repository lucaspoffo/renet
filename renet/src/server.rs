...
use log::error;
use rechannel::{error::DisconnectionReason, server::RechannelServer, Bytes};
...
pub struct RenetServer {
    socket: UdpSocket,
    reliable_server: RechannelServer,
...
impl RenetServer {
    pub fn new(
        ...
        let buffer = vec![0u8; 2000].into_boxed_slice();
        let bandwidth_smoothing_factor = 0.05;
...
fn handle_server_result(
    ...
    bandwidth_smoothing_factor: f32,
    socket: &UdpSocket,
    reliable_server: &mut RechannelServer,
