...
use web_sys::{console, Blob, BlobPropertyBag, MessageEvent, Url, WebTransport, WebTransportError, Worker, WritableStreamDefaultWriter};
...
impl WebTransportClient {
    pub async fn new(url: &str, options: Option<WebTransportOptions>) -> Result<Self, WebTransportError> {
        ...
        let v = worker.post_message_with_transfer(&web_transport.datagrams().readable(), &transfer_array);

        if v.is_err() {
            console::log_1(&v.err().unwrap());
        }
        ...
        let writer = web_transport.datagrams().writable().get_writer().unwrap();
