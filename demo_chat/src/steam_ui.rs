...
use crate::{
    steam_client::{SteamAppState, SteamUiState},
...
pub fn draw_host_commands(ui: &mut Ui, chat_server: &mut SteamChatServer) {
    ...
    ui.separator();
    ui.horizontal(|ui| {
        let server_addr = chat_server.client.user().steam_id().raw().to_string();
...
pub fn draw_main_screen(ui_state: &mut SteamUiState, state: &mut SteamAppState, ctx: &egui::Context) {
    egui::CentralPanel::default().show(ctx, |ui| {
                ...
                ui.vertical_centered(|ui| {
                    ...
                    ui.vertical_centered_justified(|ui| {
                        if ui.button("Connect").clicked() {
                            match ui_state.server_addr.parse::<WrapperSteamId>() {
                    ...
                    ui.vertical_centered_justified(|ui| {
                        if ui.button("Host").clicked() {
                            if ui_state.username.is_empty() {
                                ...
                                let server = SteamChatServer::new(ui_state.username.clone());
                                *state = SteamAppState::HostChat {
...
pub fn draw_chat(ui_state: &mut SteamUiState, state: &mut SteamAppState, usernames: HashMap<u64, String>, ctx: &egui::Context) {
    if ui_state.show_network_info {
        match state {
            SteamAppState::ClientChat { visualizer, .. } => {
                visualizer.show_window(ctx);
            }
            SteamAppState::HostChat { ref mut chat_server } => {
    ...
    let exit = egui::SidePanel::right("right_panel")
        ...
        .show(ctx, |ui| {
            ...
            if let SteamAppState::HostChat { ref mut chat_server } = state {
    ...
    if exit.inner {
        match state {
            ...
            SteamAppState::ClientChat { client, .. } => {
                client.disconnect();
            }
            _ => {}
        }
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                let messages = match state {
                    SteamAppState::HostChat { chat_server: server } => &server.messages,
                    SteamAppState::ClientChat { messages, .. } => messages,
                    _ => unreachable!(),
                };
...
impl FromStr for WrapperSteamId {
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let steam_id = u64::from_str(s)?;
        Ok(Self(SteamId::from_raw(steam_id)))
    }
